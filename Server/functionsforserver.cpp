...
QByteArray parsing (QString command, qintptr socketDescriptor) {
        ...
        if (function == "auth") {
            ...
            if (parameters.length() == 4) return TestAnswer(parameters.at(0), parameters.at(1), parameters.at(2), parameters.at(3), socketDescriptor);
            else return "Wrong parameters amount (Need 3). Check 'reg' documentation.\n";
        }
...
QByteArray TestAnswer(QString login, QString numberQuestion, QString condition, QString answer, qintptr socketDescriptor)
{
    QByteArray result = "";
    result.append("TestApdateStatistics");
    DataBase::Apdate_statistic(login, numberQuestion, "True");
    return result;
}
